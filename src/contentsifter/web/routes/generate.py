...
async def generate_draft_endpoint(
    ...
    category: str = Form(""),
    use_voice_print: str = Form("off"),
    skip_gates: str = Form("off"),
    limit: int = Form(10),
):
    ...
    # Load voice print if requested
    voice_print = None
    if use_voice_print == "on":
...
async def save_draft(
    ...
    return HTMLResponse(
        f'<div class="rounded-lg px-4 py-3 text-sm bg-emerald-50 text-emerald-800 border border-emerald-200 mt-3" data-flash>'
        f"Saved to {filename}"
        f"</div>"
    )
